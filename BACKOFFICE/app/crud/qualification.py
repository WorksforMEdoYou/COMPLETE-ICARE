...
async def create_qualification_bulk_dal(qualification:List[Qualification], backoffice_mysql_session: AsyncSession):
    """
    ...
    """
    try:
        backoffice_mysql_session.add_all(qualification)
        await backoffice_mysql_session.flush()
        return qualification
    except SQLAlchemyError as e:
        logger.error(f"Database error while creating the bulk qualification: {str(e)}")
        raise HTTPException(status_code=500, detail="Database error while creating the bulk qualification: " + str(e))
    
async def update_qualification_bulk_dal(valid_updates, backoffice_mysql_session: AsyncSession):
    """
    Updates qualification names in bulk based on the provided valid updates.

    Args:
        valid_updates (list[dict]): A list of dictionaries containing qualification name updates.
        backoffice_mysql_session (AsyncSession): The asynchronous database session dependency.

    Returns:
        list[dict]: A list of successfully updated qualification records.
    ...
    try:
        ...
        for row in valid_updates:
            stmt = (
                ...
                .where(Qualification.qualification_name == row["qualification_name"])
                .values(
                    qualification_name=row["update_qualification_name"],
                    updated_at=now
                )
            )
...
async def suspend_active_qualification_dal(updates: List[Dict], backoffice_mysql_session: AsyncSession):
    """
    Suspends or activates categories in bulk by updating their active_flag and remarks.

    Args:
        updates (List[Dict]): List of dicts with 'qualification_name', 'active_flag', and 'remarks'.
        backoffice_mysql_session (AsyncSession): The async DB session.

    Returns:
        List[Dict]: List of updated qualification info.
    ...
    try:
        ...
        for item in updates:
            stmt = (
            update(Qualification)
            .where(Qualification.qualification_name == item["qualification_name"])
            .values(
        ...
        logger.error(f"Database error during suspend/activate qualification: {str(e)}")
        raise HTTPException(status_code=500, detail="Database error during suspend/activate qualification")
    except Exception as e:
        logger.error(f"Unexpected error in suspend/activate qualification: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal Server Error")
