...
from ..crud.category import create_category_bulk_dal, update_category_bulk_dal, suspend_active_category_dal
...
async def create_category_bulk_bl(file, backoffice_mysql_session: AsyncSession):
    ...
    try:
        ...
        for chunk in pd.read_csv(path, chunksize=500):
            if 'category_name' not in chunk.columns:
                raise HTTPException(status_code=400, detail="CSV must contain 'category_name' column.")
